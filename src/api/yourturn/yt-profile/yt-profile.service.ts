...
export class YtProfileService {
  constructor(
    ...
    private accountRepository: YtAccountRepository,
  ...
  async create(createProfileDto: YtCreateProfileDto) {
    const account = await getRepository(YtAccount, 'angular').findOneOrFail(
      createProfileDto.account_id,
    );
    const roles: YtRole[] = await getRepository(YtRole, 'angular').findByIds(
      createProfileDto.roles,
    );
